...
class World {
  ...
  buildMesh(cx, cz) {
    ...
    const geo = { opaque: this._newGeoAcc(), alpha: this._newGeoAcc(), water: this._newGeoAcc() };
    const ts = 1 / ATLAS_TILES;

    const pushFace = (acc, face, x, y, z, tile, shade, hTop = 1) => {
      const base = acc.pos.length / 3;
      const tu = (tile % ATLAS_TILES) * ts, tv = 1 - (Math.floor(tile / ATLAS_TILES) + 1) * ts;
      for (const c of face.corners) {
        const py = c.pos[1] === 1 ? hTop : 0;
    ...
    for (let ly = 0; ly < HEIGHT; ly++) for (let lz = 0; lz < CHUNK; lz++) for (let lx = 0; lx < CHUNK; lx++) {
      ...
      for (const face of FACES) {
        ...
        const acc = def.fluid ? geo.water : (def.transparent ? geo.alpha : geo.opaque);
        const hTop = def.fluid && this.getBlock(wx, ly + 1, wz) !== id ? 0.875 : 1;
        pushFace(acc, face, wx, ly, wz, tile, Math.min(1.6, face.shade * glow), hTop);
      }
    }
    ...
    const set = {};
    for (const kind of ['opaque', 'alpha', 'water']) {
      const acc = geo[kind];
